...
namespace QuickScripts
{
    ...
    public class QuickSpawner : MonoBehaviour
    {
        ...
        /// <summary>
        /// Runs the spawn function. isActive must be true for this to run.
        /// </summary>
        public void Spawn()
        {
            RespawnItem();
        ...
        private void RespawnItem() // Starts the Spawn process. The next step is to find out if it's spawning to a random spawn point or ALL spawn points
        {
            // Check if User has not allocated any objects to spawn
            if (objectsToSpawn.Count == 0)
            {
                Debug.LogError(this.name + " | Quick Spawner has no object to spawn or a null object slot.", this.gameObject);
                return;
            }
        ...
        void SpawnToSpawnPoint() // Immediately spawn to one free spawn point, used for random selection of spawn points
        {
        ...
        void SpawnToAllSpawnPoints() // Immediately spawn to all spawn points that are free
        {
            for (int i = 0; i < spawnPoints.Count; i++)
            //foreach (QS_SpawnPoint spawn in spawnPoints)
            {
