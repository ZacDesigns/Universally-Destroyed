...
namespace QuickScripts
{
    ...
    public class QuickTeleport : MonoBehaviour
    {
        ...
        [Tooltip("If you want this teleport to temporarily deactivate other teleports when used, add them here.")]
        public List<QuickTeleport> cooldownOtherTeleports = new List<QuickTeleport>();
        [Space(10)]
        ...
        void ErrorMessages()
        {
            if (destinationNodes.Count == 0)
                Debug.LogError(this.name + " | Quick Teleporter has no destination!", this.gameObject);
        ...
        void OnTriggerEnter(Collider other)
        {
            if (isActive && !activated && interactableTags.Contains(other.tag.ToString()))
            {
                ...
                // Cooldown any other Teleporters in the list
                if (cooldownOtherTeleports.Count > 0)
                {
                    foreach (QuickTeleport otherTeleport in cooldownOtherTeleports)
                    {
                        otherTeleport.activated = true;
                        otherTeleport.StartCoolDown();
                    }
                }
