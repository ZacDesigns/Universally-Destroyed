...
namespace QuickScripts
{
    ...
    public class QuickTrigger : MonoBehaviour
    {
		...
		[Tooltip ("Delay in seconds before the events will fire")]
		...
		// For handling OnStay being in Physics update
		bool stayInput;

		[Space (10)]
		[Tooltip ("If Editor performance is slow, turn this off")]
		// For the Gizmo connecting lines
		...
		void OnDrawGizmos(){
			...
			// Enter Events
			for (int i = 0; i < enterEvents; i++) {
                UnityEngine.Object obj = TriggerEnter.GetPersistentTarget(i);
			...
			// Stay Events
			for (int i = 0; i < stayEvents; i++) {
                UnityEngine.Object obj = TriggerStay.GetPersistentTarget(i);
                GameObject gameObj = obj as GameObject;
                Component component = obj as Component;
			...
			// Exit Events
			for (int i = 0; i < exitEvents; i++) {
                UnityEngine.Object obj = TriggerExit.GetPersistentTarget(i);
                GameObject gameObj = obj as GameObject;
                Component component = obj as Component;
		...
		void Update()
		{
			...
			if (triggerIsActive && stayEvents > 0 && stayInput == false){ // listen for input on every frame ONLY if OnStay is being used
				stayInput = InputCheck (); // stayInput gets set to false after OnStay has run
			}
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            {
				// Quick thing to stop OnStay input from being pressed before the user enters the trigger and staying there
				stayInput = false;

                if (enterEvents > 0)
                {
					StartCoroutine (DelayOnEnter ());
                }
		...
		IEnumerator OnTriggerStay(Collider other) // This gets checked on physics update
		{
			if (triggerIsActive && interactableTags.Contains (other.tag)) {
				if (stayInput) {
					StartCoroutine (DelayOnStay ());
					stayInput = false;
				}
				else if (!requireAxisInput && !requireMouseInput && !requireKeyInput) { // No input needed, Just do it
					if (stayEvents > 0) {
						StartCoroutine (DelayOnStay ());
					}
				}
